...
const fetchData = async () => {
    try {
        const response = await $httpClient.request(myRequest);
        ...
        if (prices.length!== 4) {
            ...
            $notification.post("油价查询", `${friendly_tips}`, content);
            console.log(`油价查询结果：\n${content}`);
            return;
        }
    } catch (reason) {
        console.log(`查询油价信息失败, URL=${query_addr}`);
        $notification.post("油价查询", "请求失败", "请检查网络或反馈给开发者");
        return;
    }
};

// 根据需要调用 fetchData 函数，可以在 Loon 的规则触发时调用。
// 例如：可以在 Loon 的网络请求拦截规则中调用此函数进行油价查询。
fetchData();
